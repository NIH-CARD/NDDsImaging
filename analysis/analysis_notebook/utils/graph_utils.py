...
def draw_pval_right(
    ...
    **kwargs: Any
    ...
    if pval is not None:
        ...
        # same height (using negative_padding) and fontsize (using kwargs.get("fontsize", 8)) as the
        # ylabel height and fontsize. See draw_ylabel1(...).
        
        print (fontsize)
        ylabel1_size = kwargs.get("ylabel1_size", fontsize)
        if annoteheaders:
        ...
        if pval_title is not None:
            ...
            if annoteheaders is not None:  # if tableheaders exist
                ...
                pval_title_fontsize = kwargs.get("pval_title_fontsize", 8)
...
def format_grouplabels(
    ...
    -------
            Matplotlib Axes object.
    """
    grouplab_size = kwargs.get("grouplab_size", 8)
    grouplab_fontweight = kwargs.get("grouplab_fontweight", "bold")
            ...
            for gr in dataframe[groupvar].unique():
                try:
                    if gr.lower() == ylabel.get_text().lower().strip():
                        ...
                        ax.get_yticklabels()[ix].set_fontfamily("Times New Roman")
...
def format_tableheader(
    ...
    **kwargs: Any
    ...
    if (annoteheaders is not None) or (right_annoteheaders is not None):
        ...
        tableheader_fontsize = kwargs.get("fontsize", 8)
...
def format_xlabel(xlabel: str, ax: Axes, **kwargs: Any) -> Axes:
    ...
    if xlabel is not None:
        xlabel_size = kwargs.get("xlabel_size", 8)
...
def format_xticks(
    ...
    **kwargs: Any
    ...
    for xticklab in ax.get_xticklabels():
        xticklab.set_fontfamily("Times New Roman")
    return ax
...
def draw_tablelines(
    ...
    **kwargs: Any
    ...
    # import pdb; pdb.set_trace()
    # ax.plot(
        # [x0 * 1e-5 * 0.5, x1], [nrows - 0.4, nrows - 0.4], color="0", linewidth=upper_lw, clip_on=False
    #    [x0 * 1e-2, x1], [nrows - 0.4, nrows - 0.4], color="0", linewidth=upper_lw, clip_on=False
    # )
    # ax.plot(
    #     [x0 * 1e-2 * 0.5, x1], [nrows - 1.45, nrows - 1.45], color="0.5", linewidth=lower_lw, clip_on=False
        # [x0 * 1e-5 * 0.5, x1], [nrows - 1.45, nrows - 1.45], color="0.5", linewidth=lower_lw, clip_on=False
    # )
